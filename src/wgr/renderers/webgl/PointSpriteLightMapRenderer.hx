...
class PointSpriteLightMapRenderer implements IRenderer
{
    ...
    public function AddSpriteToBatch(x:Float,y:Float,alpha:Int,red:Int,green:Int,blue:Int) {
        var index = indexRun * 3;
        data[index+0] = Std.int(x + camera.position.x);
        data[index+1] = Std.int(y + camera.position.y);        
        index *= 4;
        data8[index+8] = red;
        data8[index+9] = blue;
        data8[index+10] = green;
        data8[index+11] = alpha;
        indexRun++;
    }
    ...
    public function Render(clip:AABB) {
        ...
        gl.vertexAttribPointer(untyped pointSpriteShader.attribute.position, 2, RenderingContext.FLOAT, false, 12, 0);
        gl.vertexAttribPointer(untyped pointSpriteShader.attribute.colour, 4, RenderingContext.UNSIGNED_BYTE, true, 12, 8);

        gl.uniform2f(untyped pointSpriteShader.uniform.projectionVector,projection.x,projection.y);
        gl.uniform1f(untyped pointSpriteShader.uniform.size,32);             

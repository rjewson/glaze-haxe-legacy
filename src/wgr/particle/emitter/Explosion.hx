...
class Explosion implements IParticleEmitter
{
    ...
    public function update(time:Float, position:Vector2D, engine:IParticleEngine):Void {
        for (i in 0...mass) {
            ...
            var p = utils.Random.RandomFloat(0,power);
            var vx = Math.cos(angle) * p;
            var vy = Math.sin(angle) * p;
            engine.EmitParticle(position.x,position.y,vx,vy,0,0,utils.Random.RandomInteger(300,700),0.99,true,true,null,4,255,255,0,0);
        }
    }

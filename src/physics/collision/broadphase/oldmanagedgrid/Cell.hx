...
class Cell 
{
	...
	public var dynamicItems:FastList<Body>;
	public var sleepingItems:FastList<Body>;
	public var staticItems:FastList<Body>;
	...
	public function new(manager:ManagedGrid, index:Int, x:Float, y:Float, w:Float, h:Float) 
	{
		...
		dynamicItems = new FastList<Body>();
		sleepingItems = new FastList<Body>();
		staticItems = new FastList<Body>();
	...
	function SearchList(list:FastList<Body>, action: IBroadphaseAction, actionResultCollection:ActionResultCollection):Void {
		for (body in list) {
